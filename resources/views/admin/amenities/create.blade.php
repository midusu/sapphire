...
@section('title', 'Add Amenity - Sapphire Hotel')
@section('header', 'Add Amenity')
    ...
    <form method="POST" action="{{ route('admin.amenities.store') }}" enctype="multipart/form-data">
        ...
        <div class="bg-white rounded-lg shadow p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="icon" value="{{ old('icon') }}" placeholder="e.g., fas fa-wifi"
                    class="w-full border-gray-300 rounded-lg">
                <p class="text-xs text-gray-500 mt-1">Leave empty if using image</p>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Image</label>
                <input type="file" name="image" accept="image/*"
                    class="w-full border-gray-300 rounded-lg">
                <p class="text-xs text-gray-500 mt-1">Optional - use if no icon</p>
            </div>
            <div>
