...
@section('title', 'Edit Food Item - Sapphire Hotel Management')
@section('header', 'Edit Food Item: ' . $food->name)
...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <form action="{{ route('admin.food.items.update', $food) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="category" class="w-full border rounded-lg px-3 py-2" required>
                            ...
                            <option value="appetizer" {{ old('category', $food->category) == 'appetizer' ? 'selected' : '' }}>
                                Appetizer</option>
                            <option value="main_course" {{ old('category', $food->category) == 'main_course' ? 'selected' : '' }}>Main Course</option>
                            <option value="dessert" {{ old('category', $food->category) == 'dessert' ? 'selected' : '' }}>
                                Dessert</option>
                            <option value="beverage" {{ old('category', $food->category) == 'beverage' ? 'selected' : '' }}>
                                Beverage</option>
                    ...
                    <div>
                        ...
                        <input type="number" name="preparation_time"
                            value="{{ old('preparation_time', $food->preparation_time) }}"
                            class="w-full border rounded-lg px-3 py-2" required>
                    ...
                    <div>
                        ...
                        <select name="menu_type" class="w-full border rounded-lg px-3 py-2" required>
                            <option value="both" {{ old('menu_type', $food->menu_type ?? 'both') == 'both' ? 'selected' : '' }}>Remote Service & Restaurant</option>
                            <option value="room_service" {{ old('menu_type', $food->menu_type ?? 'both') == 'room_service' ? 'selected' : '' }}>Room Service Only</option>
                            <option value="restaurant" {{ old('menu_type', $food->menu_type ?? 'both') == 'restaurant' ? 'selected' : '' }}>Restaurant Only</option>
                        </select>
                    ...
                    <div class="col-span-2">
                        ...
                        <input type="url" name="image_url" value="{{ old('image_url', $food->image_url) }}"
                    ...
                    <div class="col-span-2">
                        <label class="flex items-center space-x-2">
                            <input type="checkbox" name="available" value="1" {{ old('available', $food->available) ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                    ...
                    <div class="col-span-2 border-t pt-4 mt-2">
                        ...
                        <select name="inventory_item_id" class="w-full border rounded-lg px-3 py-2">
                            ...
                            @foreach($inventoryItems as $item)
                                <option value="{{ $item->id }}" {{ old('inventory_item_id', $food->inventory_item_id) == $item->id ? 'selected' : '' }}>
